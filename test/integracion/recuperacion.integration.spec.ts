...
describe('PRUEBAS DE INTEGRACIÓN - Recuperación de Contraseña (RF-003)', () => {
   ...
   * Tipo: INTEGRACIÓN
   * Objetivo: Token expirado de rec. - Invalidation tras tiempo límite.
   * Criterio de Aceptación: Lanzar BadRequestException si el token o tiempo de solicitud ha caducado.
   * --------------------------------------------------------------------------
   */
  describe('CP-082: Token expirado de rec.', () => {
    it('Debe rechazar la solicitud de restablecimiento si el token ha expirado', async () => {
      const validateRecoveryToken = (tokenCreationDate: Date) => {
        const MAX_TIME_MS = 15 * 60 * 1000; // 15 minutos
      ...
      const expiredDate = new Date(Date.now() - 20 * 60 * 1000); // Hace 20 min
   ...
   * Requerimiento: RF-003
   * Tipo: INTEGRACIÓN
   * Objetivo: Auditoría restablecimiento - Log en BD del cambio de clave.
   * Criterio de Aceptación: Confirmar el registro automático de auditoría tras restablecer la contraseña.
   * --------------------------------------------------------------------------
   */
