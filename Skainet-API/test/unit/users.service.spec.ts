...
describe('UsersService - Pruebas Unitarias', () => {
  ...
  it('1. findAll: Debe retornar todos los usuarios excluyendo la contraseña y parseando JSON (CP-005, CP-016)', async () => {
  ...
  it('2. findOne: Debe buscar un usuario sin exponer las respuestas de seguridad (CP-006)', async () => {
  ...
  it('5. updateStatus: Debe actualizar el estado y registrar la entrada en el historial (CP-010, CP-020)', async () => {
    prismaMock.user.findUnique.mockResolvedValue(rawUser);
  ...
  it('7. create: Debe crear un nuevo usuario y validar campos obligatorios y duplicidad (CP-001..CP-003, CP-013..CP-014)', async () => {
    prismaMock.user.findUnique.mockResolvedValue(null);
  ...
  it('8. validateRecovery: Debe validar respuestas de recuperación sin importar mayúsculas o espacios (CP-028..CP-031)', async () => {
    prismaMock.user.findUnique.mockResolvedValue(rawUser);
