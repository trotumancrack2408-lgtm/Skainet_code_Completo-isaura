...
export default function InventoryPanel({ API_URL, token, currentUser, activeOrders = [] }) {
  ...
  const isScaleQuantity = (value) => {
    const quantity = Number(value);
    return Number.isFinite(quantity) && Math.abs(quantity * 10 - Math.round(quantity * 10)) < Number.EPSILON * 10;
  };

  const validateScaleQuantity = (value, label, allowZero = false) => {
    const quantity = Number(value);
    if (!Number.isFinite(quantity) || (allowZero ? quantity < 0 : quantity <= 0)) {
      alert(`${label} debe ser ${allowZero ? 'mayor o igual a cero' : 'mayor a cero'}.`);
      return false;
    }
    if (!isScaleQuantity(value)) {
      alert(`${label} solo admite incrementos de 0.1; la báscula no mide una precisión mayor.`);
      return false;
    }
    return true;
  };
  ...
  const handleCreateMaterial = async (e) => {
    ...
    if (!validateScaleQuantity(matStockInicial, 'El stock inicial', true)
      || !validateScaleQuantity(matMinStock, 'El stock mínimo', true)) return;
  ...
  const handleRegisterEntry = async (e) => {
    ...
    if (!validateScaleQuantity(movQty, 'La cantidad a ingresar')) return;
  ...
  const handleRegisterSalida = async (e) => {
    ...
    if (!validateScaleQuantity(movQty, 'La cantidad a descontar')) return;
            ...
            <form onSubmit={handleCreateMaterial} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div style={{ display: 'flex', gap: '12px' }}>
                <div style={{ flex: 1 }}>
                  ...
                  <input type="number" step="0.1" value={matStockInicial} onChange={(e) => setMatStockInicial(e.target.value)} style={{ width: '100%', padding: '10px', backgroundColor: '#111827', border: '1px solid #374151', borderRadius: '6px', color: '#fff' }} />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={{ fontSize: '0.85rem', color: '#9ca3af' }}>Stock Mínimo (Alerta)</label>
                  <input type="number" step="0.1" value={matMinStock} onChange={(e) => setMatMinStock(e.target.value)} style={{ width: '100%', padding: '10px', backgroundColor: '#111827', border: '1px solid #374151', borderRadius: '6px', color: '#fff' }} />
                </div>
              </div>
          ...
          <div style={{ backgroundColor: '#1f2937', padding: '24px', borderRadius: '12px', width: '100%', maxWidth: '440px', border: '1px solid #374151' }}>
            ...
            <form onSubmit={handleRegisterEntry} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div>
                ...
                <input type="number" step="0.1" required value={movQty} onChange={(e) => setMovQty(e.target.value)} placeholder="0.0" style={{ width: '100%', padding: '10px', backgroundColor: '#111827', border: '1px solid #374151', borderRadius: '6px', color: '#fff' }} />
                <small style={{ color: '#9ca3af' }}>La báscula registra cantidades en incrementos de 0.1.</small>
              </div>
